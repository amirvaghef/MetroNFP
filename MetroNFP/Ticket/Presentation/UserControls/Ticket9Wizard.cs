...
{
    public partial class Ticket9Wizard : MetroNFP.Base
    {
        Common.Ticket ticket = new Common.Ticket();
        public Ticket9Wizard()
        {
            InitializeComponent();
        ...
        private void BindDropDowns()
        {
            ...
            if (ticket.fk_BankCostSlotProfileID != 0)
            {
                cbBankHoliday.SelectedValue = ticket.fk_BankCostSlotProfileID;
                cbRegularDays.SelectedValue = ticket.fk_RegularCostSlotProfileID;
                cbSchoolHoliday.SelectedValue = ticket.fk_SchoolCostSlotProfileID;
            }
        }
        ...
        private object Submit()
        {
            if (cbRegularDays.SelectedIndex != -1)
            {
                if (cbSchoolHoliday.SelectedIndex != -1)
                {
                    ...
                    {
                        ticket.fk_BankCostSlotProfileID = short.Parse(cbBankHoliday.SelectedValue.ToString());
                        ticket.fk_RegularCostSlotProfileID = short.Parse(cbRegularDays.SelectedValue.ToString());
                        ticket.fk_SchoolCostSlotProfileID = short.Parse(cbSchoolHoliday.SelectedValue.ToString());
                    }
                    else
                    {
